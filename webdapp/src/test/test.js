...
/*
describe("JulzPay unsiswap implementation should work", function(){
    let owner;
    let treasury;
    let monthly = true;
    ...
    it("should deposit dai and swap to weth", async function(){
        //first deposit dai directly into the contract address
        const path = encodePath([DAI_ADDR, WETH_ADDR], [3000]);
                ...
                try{
                    await dai.connect(depositorSigner).transfer(contract.address, deposit); 
                    currentDepositBalance = await dai.balanceOf(contract.address);
                    //call swap
                    await contract.swap(DAI_ADDR, path, currentDepositBalance);
                } catch(err){
                    console.log("Error:",err);
                }                             
                
        //verify swapped token balance
        const ethbal = await ethers.provider.getBalance(contract.address); 
        const wethbal = await weth.balanceOf(contract.address);
        console.log(ethbal,wethbal);
    });

   /* it("should deposit eth and swap to dai", async function(){
        //first deposit dai directly into the contract address
        const path = encodePath([WETH_ADDR,DAI_ADDR], [3000]);
                ...
                try{
                    ...
                    ethers.provider.sendTransaction(tx);
                    addr1.transfer(contract.address,deposit);
                    currentDepositBalance = await ethers.provider.getBalance(contract.address); 
                    console.log('PMMMMMM',currentDepositBalance);
                    //call swap
                    await contract.swap(WETH_ADDR, path, currentDepositBalance);
                } catch(err){
                    console.log("Error:",err);
                }                             
        ...
        //verify swapped token balance
        const ethbal = await ethers.provider.getBalance(contract.address); 
        const wethbal = await dai.balanceOf(contract.address);
        const f = await dai.balanceOf(contract.address);
        console.log(ethbal,wethbal,f);
    });

    it("should deposit usdc and swap to dai", async function(){
        //first deposit dai directly into the contract address
        //call swap
        //verify swapped token balance
    });

    it("should deposit usdc and swap to eth", async function(){
        //first deposit dai directly into the contract address
        //call swap
        //verify swapped token balance
    });
